...
class GoGNN(nn.Module):
    def __init__(self, stone_in=18, string_in=2, global_in=19,
        ...
        def make_hetero_conv_extract(heads):
            return HeteroConv({
        ...
        def make_hetero_conv_reason(heads):
            return HeteroConv({
                ('stone', 'adjacent', 'stone'): GATv2Conv(stone_dim, stone_dim // heads, heads=heads),
                ('stone', 'belongs_to', 'string'): GATv2Conv((stone_dim, hyper_dim), hyper_dim // heads, heads=heads, add_self_loops=False),
                ('string', 'contains', 'stone'): GATv2Conv((hyper_dim, stone_dim), stone_dim // heads, heads=heads, add_self_loops=False),
                ('string', 'adjacent_to', 'string'): GATv2Conv(hyper_dim, hyper_dim // heads, heads=heads),
                # String does not report to the global node during reasoning to avoid over smoothing
                ('global_node', 'influences', 'string'): GATv2Conv(hyper_dim, hyper_dim // heads, heads=heads, add_self_loops=False),
            }, aggr='sum')
        ...
        self.extraction_layers = nn.ModuleList([make_hetero_conv_extract(extraction_heads) for _ in range(8)])
        ...
        # Block A
        self.conv_A = make_hetero_conv_reason(reasoning_heads)
        self.ffn_stone_A = make_ffn(stone_dim)
        self.ffn_hyper_A = make_ffn(hyper_dim)
        
        # Block B
        self.conv_B = make_hetero_conv_reason(reasoning_heads)
    ...
    def forward(self, x_dict, edge_index_dict, string_batch_index):
        ...
        # --- Value Head Assembly ---
        batch_size = x_dict['global_node'].size(0)
        string_pool = global_mean_pool(x_dict['string'], string_batch_index, size=batch_size)
        
        # Concatenate [Batch, Hyper] + [Batch, Hyper] -> [Batch, Hyper*2]
        v_input = torch.cat([x_dict['global_node'], string_pool], dim=-1)

        return (self.policy_head(x_dict['stone']).squeeze(-1),
                self.pass_head(x_dict['global_node']).squeeze(-1),
                self.value_head(v_input).squeeze(-1),
